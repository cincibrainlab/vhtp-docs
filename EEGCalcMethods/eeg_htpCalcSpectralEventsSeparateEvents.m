function [EEG, results] = eeg_htpCalcSpectralEventsSeparateEvents(EEG, varargin)
...
%   'outputdir' - path for saved output files 
%                 default: tempdir
%
% Outputs:
    ...
    for bi = fieldnames(se_tmp)'
        ...
        se_IEI = se_band_tmp.("IEI");
         ...
         for ei = 1:length(se_Events.Events.trialind)
            ...
            features = {'trialind','time_withintrial', 'eventduration', 'eventpower', ...
                'fspan', 'onsettiming', 'offsettiming', 'maximapowerFOM', 'time_adjusted', 'maximafreq'}; %Fields within specEv_struct

            for fi = features
                switch fi{1}
                    case 'trialind' % trial number/index for current event
                        csvout{count, 4} = ...
                            se_Events.Events.trialind(ei);
                    case 'time_withintrial'
                        csvout{count, 5} = ...
                            se_Events.Events.maximatiming(ei);
                    case 'eventduration'
                        csvout{count, 6} = ...
                            se_Events.Events.duration(ei);
                    case 'eventpower'
                        csvout{count, 7} = ...
                            se_Events.Events.maximapower(ei); 
                    case 'fspan'
                        csvout{count, 8} = ...
                            se_Events.Events.Fspan(ei);
                    case 'onsettiming'
                        csvout{count, 9} = ...
                            se_Events.Events.onsettiming(ei);
                    case 'offsettiming'
                        csvout{count, 10} = ...
                            se_Events.Events.offsettiming(ei);
                    case 'maximapowerFOM'
                        csvout{count, 11} = ...
                            se_Events.Events.maximapowerFOM(ei);
                    case 'time_adjusted'
                        csvout{count, 12} = ...
                            se_Events.Events.maximatiming(ei) + EEG.xmin;
                    case 'maximafreq'
                        csvout{count, 13} = ...
                            se_Events.Events.maximafreq(ei);
                end
            end
            count = count + 1; % row number
           
        end
    end
end

columnNames = {'eegid', 'band', 'channel'...
    'trialind','time_withintrial','eventduration', 'eventpower', ...
     'fspan', 'onsettiming', 'offsettiming', 'maximapowerFOM', 'time_adjusted', 'maximafreq'}; %Fields within specEv_struct
