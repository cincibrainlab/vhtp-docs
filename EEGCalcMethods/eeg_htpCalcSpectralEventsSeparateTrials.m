function [EEG, results] = eeg_htpCalcSpectralEventsSeparateTrials(EEG, varargin)
...
for ci = fieldnames(chanSpectralEvents)'
    ...
    for bi = fieldnames(se_tmp)'
        ...
        features = {'trialind', 'eventnumber', 'meaneventpower', 'meaneventduration', 'meaneventFspan', 'meanpower'};

        % loop through trials
        for trial_i = 1 : se_TrialSummary.NumTrials

            % trial feature summary table
            csvout{count, 1} = EEG.setname;
            csvout{count, 2} = bi{1};
            csvout{count, 3} = ci{1};
            ...
            for fi = features
                switch fi{1} 
                    ...
                    case 'eventnumber'
                        csvout{count, 5} = ...
                            se_TrialSummary.TrialSummary.eventnumber(trial_i);
                    case 'meaneventpower' 
                        csvout{count, 6} = ...
                            se_TrialSummary.TrialSummary.meaneventpower(trial_i);
                    case 'meaneventduration' 
                        csvout{count, 7} = ...
                            se_TrialSummary.TrialSummary.meaneventduration(trial_i);
                    case 'meaneventFspan' 
                        csvout{count, 8} = ...
                            se_TrialSummary.TrialSummary.meaneventFspan(trial_i);
                    case 'meanpower'
                        csvout{count, 9} = ...
                            se_TrialSummary.TrialSummary.meanpower(trial_i);
                end
            end
            count = count + 1;
...
columnNames = {'eegid', 'band', 'channel', ...
    'trialind', 'eventnumber', 'meaneventpower', 'meaneventduration', 'meaneventFspan', 'meanpower'}; %Fields within specEv_struct
